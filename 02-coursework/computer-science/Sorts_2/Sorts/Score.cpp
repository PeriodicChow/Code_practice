...
void TestScore()
{
	...
	// 函数指针数组，指向带统计参数的排序函数
	void (*f[])(Score*, int, long long&, long long&) = {Bubble, Select, Qsort};  //***
	int n, m = sizeof(f)/sizeof(*f);						// m为函数指针数组f的元素个数（此处有3个函数指针）
	bool flag;
	double t;
	long long compareCount, assignCount;  //***
	
	cout << "\n原始数据分布: 正态分布, 数据类型 = " << typeid(Score).name() << endl;
	cout << "数据规模\t时间(秒)\t比较次数\t赋值次数\t算法名称\t状态" << endl;
	cout << "--------------------------------------------------------------------------------" << endl;
	...
	for(n=1024; n<=65536; n*=2)
	{
		...
		for(int i=0; i<m; i++)
		{
			...
			gettime(1);										// 设置计时起点
			f[i](data, n, compareCount, assignCount);		// 第 i 种排序算法，传递统计参数。****
