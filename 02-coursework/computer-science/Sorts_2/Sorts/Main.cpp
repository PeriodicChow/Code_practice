...
template <typename T> void Test(const T &x)					// 调用本函数的实参的值不重要，重要的是实参的数据类型
{
	...
	char InitConf[][30] = {"Reversed", "Sorted", "Uniform", "Normal"};	// C-字符串数组，增大到30
	char algo[][30] 	= {"Bubble", "Select", "Quick"};				// C-字符串数组，增大到30
	// 函数指针数组，指向带统计参数的排序函数
	void (*f[])(T*, int, long long&, long long&) = {Bubble, Select, Qsort};
	int i, j, n, m = sizeof(f)/sizeof(*f);					// m为函数指针数组f的元素个数
	bool flag[20];											// 记录不同算法执行的正确性
	double t[20];											// 记录不同算法的执行时间
	long long compareCount[20], assignCount[20];			// 记录比较次数和赋值次数
	
	for(j=2; j>=-1; j--)									// 数据分布类型
	{
		cout << "\nData Type: " << typeid(T).name() << " (" << InitConf[j+1] << ")" << endl;
		cout << "Size";
		for(i=0; i<m; i++)
			cout << '\t' << algo[i] << "(sec)";
		cout << '\t';
		...
		for(n=1024; n<=65536; n*=2)
		{
			...
			for(i=0; i<m; i++)
			{
				...
				gettime(1);									// 设置计时起点
				f[i](data, n, compareCount[i], assignCount[i]);	// 第 i 种排序算法，传递统计参数
